...
export default factories.createCoreController('api::real-estate.real-estate', ({ strapi }) => ({
    async update(ctx) {
        try {
            ...
            const itemData = await strapi.entityService.findOne('api::real-estate.real-estate', response.data.id, {
            ...
            if (process.env.APP_ENV === "Production") {
                ...
                if (response.data.attributes &&
                    ...
                    await strapi.plugins['email'].services.email.send({
                        ...
                        <a href='${process.env.PUBLIC_URL}/admin/content-manager/collection-types/api::real-estate.real-estate/${response.data.id}'>Link to admin portal</a> `,
                    });
                    let emailToAddressUser = itemData.author.email;
                    await strapi.plugins['email'].services.email.send({
                        to: emailToAddressUser,
